...
import { ComponentStyles, colorTokens, sizingTokens, spacingTokens } from '@nectar-ui/tokens'
...
export const Seperator: SeperatorComponent = forwardRef(
	(
		{
			as = 'div',
			variant = 'horizontal',
		...
		if (color) {
			style['--seperator-color'] = color in colorTokens ? `var(--colors-${color})` : color
		}
		if (gap) {
			style['--seperator-gap'] = gap in spacingTokens ? `var(--spacings-${gap})` : gap
		}
		if (size) {
			style['--seperator-size'] = size in sizingTokens ? `var(--sizings-${size})` : size
		}
		if (styling) {
			style['--seperator-styling'] = styling
		}

		const classNames = clsx(styles.seperator, styles[alignment], styles[variant], className)
